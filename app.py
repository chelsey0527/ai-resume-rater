...
st.sidebar.title("Resume feedback generator")
...
if uploaded_file:
    ...
    with NamedTemporaryFile(delete=False) as tmp:  
        ...
        llm = OpenAI(
            ...
            model="gpt-3.5-turbo",
            temperature=0.0,
            system_prompt="You are a professional resume and cover letter corrector in tech. You are taught that a strong resume should be brief but powerful, have strong action verbs (facilitated is not a strong action verb), and show impact of the experience. You will be provided with the document of the candidate's resume or cover letter. Give detailed helpful feedback on what changes can be made to make the resume more effective. You should always tailor your response to the role the candidate is applying for. You should also provide a score out of 10 with different aspects (like structure, tech skills,Industry professionalism, and the impact of working outcomes). The score should reflect the quality of the resume for the role. Refer to the candidate as 'you' and the resume as 'your resume'.",
        )
...
# Fake user profile photo and name
st.sidebar.divider()
st.sidebar.markdown('### User: ' + " Chelsey ")
